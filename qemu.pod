...
I<cache> is "none", "writeback", or "writethrough" and controls how the host cache is used to access block data.
...
corruption.  When using the B<-snapshot> option, writeback caching is
used by default.
...
=item B<-chardev> I<backend> B<,id=>I<id> B<[,>I<options>B<]>
...
=item B<TCP options: port=>I<host> B<[,host=>I<host>B<] [,to=>I<to>B<] [,ipv4] [,ipv6] [,nodelay]>
...
=item B<-chardev stdio ,id=>I<id>

Connect to standard input and standard output of the qemu process.
B<stdio> does not take any options. B<stdio> is not available on
Windows hosts.
